...
class VIEW3D_HT_tool_header(Header):
    ...
    def draw_tool_settings(self, context):
        ...
        def draw_3d_brush_settings(layout, tool_mode):
            layout.popover(
                "VIEW3D_PT_tools_brush_settings_advanced", text="Brush")
    ...
    def draw_mode_settings(self, context):
        ...
        if mode_string == 'EDIT_ARMATURE':
            ...
            sub.prop(context.object.data, "use_mirror_x",
                     text="X", toggle=True)
        elif mode_string == 'POSE':
            _row, sub = row_for_mirror()
            sub.prop(context.object.pose, "use_mirror_x",
                     text="X", toggle=True)
        elif mode_string in {'EDIT_MESH', 'PAINT_WEIGHT', 'SCULPT', 'PAINT_VERTEX', 'PAINT_TEXTURE'}:
            # Mesh Modes, Use Mesh Symmetry
            row, sub = row_for_mirror()
            sub.prop(context.object, "use_mesh_mirror_x",
                     text="X", toggle=True)
            sub.prop(context.object, "use_mesh_mirror_y",
                     text="Y", toggle=True)
            sub.prop(context.object, "use_mesh_mirror_z",
                     text="Z", toggle=True)
            if mode_string == 'EDIT_MESH':
                ...
                row.popover(
                    panel="VIEW3D_PT_sculpt_symmetry_for_topbar", text="")
            ...
            sub.prop(context.object.data, "use_mirror_x",
                     text="X", toggle=True)
            sub.prop(context.object.data, "use_mirror_y",
                     text="Y", toggle=True)
            sub.prop(context.object.data, "use_mirror_z",
                     text="Z", toggle=True)

            layout.prop(context.object.data, "use_sculpt_collision",
                        icon='MOD_PHYSICS', icon_only=True, toggle=True)

        # Expand panels from the side-bar as popovers.
        popover_kw = {"space_type": 'VIEW_3D',
                      "region_type": 'UI', "category": "Tool"}
...
class _draw_tool_settings_context_mode:
    ...
    def SCULPT(context, layout, tool):
        ...
        layout.template_ID_preview(
            paint, "brush", rows=3, cols=8, hide_buttons=True)
    ...
    def PAINT_GPENCIL(context, layout, tool):
        ...
        if context.object and brush.gpencil_tool in {'FILL', 'DRAW'}:
            ...
            brush_basic__draw_color_selector(
                context, layout, brush, gp_settings, None)

        if context.object and brush.gpencil_tool == 'TINT':
            row.separator(factor=0.4)
            row.prop_with_popover(brush, "color", text="",
    ...
    def WEIGHT_GPENCIL(context, layout, tool):
        ...
        layout.popover(
            "VIEW3D_PT_tools_grease_pencil_weight_options", text="Options")
        layout.popover(
    ...
    def PARTICLE(context, layout, tool):
        ...
        if tool == 'ADD':
            ...
            if tool == 'LENGTH':
                ...
                row.prop(settings, "use_emitter_deflect",
                         text="Deflect Emitter")
                sub = row.row(align=True)
    ...
    def SCULPT_CURVES(context, layout, tool):
        ...
        if curves_tool == 'COMB':
            ...
            layout.prop(brush, "falloff_shape", expand=True)
            row = layout.row(align=True)
            row.prop(brush.curves_sculpt_settings,
                     "density_mode", text="", expand=True)
            row = layout.row(align=True)
            row.prop(brush.curves_sculpt_settings,
                     "minimum_distance", text="Distance Min")
            row.operator_context = 'INVOKE_REGION_WIN'
            row.operator("sculpt_curves.min_distance_edit",
                         text="", icon='DRIVER_DISTANCE')
            row = layout.row(align=True)
            row.enabled = brush.curves_sculpt_settings.density_mode != 'REMOVE'
            row.prop(brush.curves_sculpt_settings,
                     "density_add_attempts", text="Count Max")
            layout.popover("VIEW3D_PT_tools_brush_falloff")
            layout.popover("VIEW3D_PT_curves_sculpt_add_shape",
                           text="Curve Shape")
...
class VIEW3D_HT_header(Header):
    ...
    def draw_xform_template(layout, context):
        ...
        if object_mode in {'OBJECT', 'EDIT', 'EDIT_GPENCIL', 'SCULPT_GPENCIL'} or has_pose_mode:
            layout.prop(tool_settings, "transform_pivot_point",
                        text="", icon_only=True)
    ...
    def draw(self, context):
        ...
        if obj:
            ...
            if object_mode == 'PARTICLE_EDIT':
                row = layout.row()
                row.prop(tool_settings.particle_edit,
                         "select_mode", text="", expand=True)
        ...
        if obj and obj.type == 'GPENCIL' and context.gpencil_data:
            ...
            if gpd.is_stroke_paint_mode:
                row = layout.row()
                sub = row.row(align=True)
                sub.prop(tool_settings, "use_gpencil_draw_onback",
                         text="", icon='MOD_OPACITY')
                sub.separator(factor=0.4)
                sub.prop(tool_settings, "use_gpencil_automerge_strokes", text="")
                sub.separator(factor=0.4)
                sub.prop(tool_settings, "use_gpencil_weight_data_add",
                         text="", icon='WPAINT_HLT')
                sub.separator(factor=0.4)
                sub.prop(tool_settings, "use_gpencil_draw_additive",
                         text="", icon='FREEZE')
            ...
            if gpd.use_stroke_edit_mode:
                row = layout.row(align=True)
                row.prop_enum(
                    tool_settings, "gpencil_selectmode_edit", text="", value='POINT')
                row.prop_enum(
                    tool_settings, "gpencil_selectmode_edit", text="", value='STROKE')

                subrow = row.row(align=True)
                subrow.enabled = not gpd.use_curve_edit
                subrow.prop_enum(
                    tool_settings, "gpencil_selectmode_edit", text="", value='SEGMENT')
            ...
            if gpd.is_stroke_sculpt_mode:
                ...
                row.prop(tool_settings,
                         "use_gpencil_select_mask_segment", text="")
        ...
        row = layout.row()
        row.active = (object_mode == 'EDIT') or (
            shading.type in {'WIREFRAME', 'SOLID'})
...
class ShowHideMenu:
    ...
    def draw(self, _context):
        ...
        layout.operator("%s.hide" % self._operator_name,
                        text="Hide Selected").unselected = False
        layout.operator("%s.hide" % self._operator_name,
    ...
    def draw(self, context):
        ...
        if context.mode == 'EDIT_MESH':
            ...
            layout.operator("transform.transform",
                            text="Radius").mode = 'CURVE_SHRINKFATTEN'

        if context.mode != 'EDIT_CURVES':
            layout.separator()
            layout.operator("transform.translate",
                            text="Move Texture Space").texture_space = True
            layout.operator("transform.resize",
                            text="Scale Texture Space").texture_space = True
    ...
    def draw(self, context):
        ...
        if obj.type == 'ARMATURE' and obj.mode in {'EDIT', 'POSE'}:
            if obj.data.display_type == 'BBONE':
                ...
                layout.operator("transform.transform",
                                text="Scale BBone").mode = 'BONE_SIZE'
                ...
                layout.operator("transform.transform",
                                text="Scale Radius").mode = 'BONE_ENVELOPE'
...
class VIEW3D_MT_uv_map(Menu):
    ...
    def draw(self, _context):
        ...
        layout.operator("uv.project_from_view",
                        text="Project from View (Bounds)").scale_to_bounds = True
...
class VIEW3D_MT_view(Menu):
    ...
    def draw(self, context):
        ...
        layout.operator("view3d.view_selected",
                        text="Frame Selected").use_all_regions = False
        if view.region_quadviews:
            layout.operator(
                "view3d.view_selected", text="Frame Selected (Quad View)").use_all_regions = True
...
class VIEW3D_MT_view_viewpoint(Menu):
    ...
    def draw(self, _context):
        ...
        layout.operator("view3d.view_axis", text="Top",
                        text_ctxt=i18n_contexts.editor_view3d).type = 'TOP'
        layout.operator("view3d.view_axis", text="Bottom",
                        text_ctxt=i18n_contexts.editor_view3d).type = 'BOTTOM'

        layout.separator()

        layout.operator("view3d.view_axis", text="Front",
                        text_ctxt=i18n_contexts.editor_view3d).type = 'FRONT'
        layout.operator("view3d.view_axis", text="Back",
                        text_ctxt=i18n_contexts.editor_view3d).type = 'BACK'

        layout.separator()

        layout.operator("view3d.view_axis", text="Right",
                        text_ctxt=i18n_contexts.editor_view3d).type = 'RIGHT'
        layout.operator("view3d.view_axis", text="Left",
                        text_ctxt=i18n_contexts.editor_view3d).type = 'LEFT'
...
class VIEW3D_MT_view_align(Menu):
    ...
    def draw(self, _context):
        ...
        layout.operator("view3d.view_all",
                        text="Center Cursor and Frame All").center = True
...
class VIEW3D_MT_select_object_more_less(Menu):
    ...
    def draw(self, _context):
        ...
        props = layout.operator(
            "object.select_hierarchy", text="Extend Parent")
        props.extend = True
        props.direction = 'PARENT'
...
class VIEW3D_MT_select_object(Menu):
    ...
    def draw(self, _context):
        ...
        layout.operator_menu_enum(
            "object.select_linked", "type", text="Select Linked")
...
class VIEW3D_MT_select_pose(Menu):
    ...
    def draw(self, _context):
        ...
        layout.operator_menu_enum(
            "pose.select_grouped", "type", text="Grouped")
        layout.operator("object.select_pattern", text="Select Pattern...")
...
class VIEW3D_MT_edit_mesh_select_loops(Menu):
    ...
    def draw(self, _context):
        ...
        layout.operator("mesh.loop_multi_select",
                        text="Edge Loops").ring = False
        layout.operator("mesh.loop_multi_select",
                        text="Edge Rings").ring = True
...
class VIEW3D_MT_select_edit_text(Menu):
    ...
    def draw(self, _context):
        ...
        layout.operator("font.move_select",
                        text="Previous Block").type = 'PREVIOUS_PAGE'
        layout.operator("font.move_select",
                        text="Next Block").type = 'NEXT_PAGE'

        layout.separator()

        layout.operator("font.move_select",
                        text="Line Begin").type = 'LINE_BEGIN'
        layout.operator("font.move_select", text="Line End").type = 'LINE_END'

        layout.separator()

        layout.operator("font.move_select",
                        text="Previous Line").type = 'PREVIOUS_LINE'
        layout.operator("font.move_select",
                        text="Next Line").type = 'NEXT_LINE'

        layout.separator()

        layout.operator("font.move_select",
                        text="Previous Word").type = 'PREVIOUS_WORD'
        layout.operator("font.move_select",
                        text="Next Word").type = 'NEXT_WORD'
...
class VIEW3D_MT_select_edit_metaball(Menu):
    ...
    def draw(self, _context):
        ...
        layout.operator_menu_enum(
            "mball.select_similar", "type", text="Similar")
...
class VIEW3D_MT_select_edit_armature(Menu):
    ...
    def draw(self, _context):
        ...
        props = layout.operator(
            "armature.select_hierarchy", text="Extend Parent")
        props.extend = True
        props.direction = 'PARENT'

        props = layout.operator(
            "armature.select_hierarchy", text="Extend Child")
        props.extend = True
        props.direction = 'CHILD'

        layout.operator_menu_enum(
            "armature.select_similar", "type", text="Similar")
        layout.operator("object.select_pattern", text="Select Pattern...")
...
class VIEW3D_MT_paint_gpencil(Menu):
    ...
    def draw(self, _context):
        ...
        layout.operator("gpencil.vertex_color_hsv",
                        text="Hue/Saturation/Value")
        layout.operator("gpencil.vertex_color_brightness_contrast",
...
class VIEW3D_MT_select_gpencil(Menu):
    ...
    def draw(self, context):
        ...
        layout.operator_menu_enum(
            "gpencil.select_grouped", "type", text="Grouped")

        if context.mode == 'VERTEX_GPENCIL':
            layout.operator("gpencil.select_vertex_color",
...
class VIEW3D_MT_select_paint_mask(Menu):
    ...
    def draw(self, _context):
        ...
        layout.operator("paint.face_select_all",
...
class VIEW3D_MT_select_paint_mask_vertex(Menu):
    ...
    def draw(self, _context):
        ...
        layout.operator("paint.vert_select_ungrouped",
...
class VIEW3D_MT_mesh_add(Menu):
    ...
    def draw(self, _context):
        ...
        layout.operator("mesh.primitive_grid_add",
...
class VIEW3D_MT_curve_add(Menu):
    ...
    def draw(self, context):
        ...
        layout.operator("curve.primitive_bezier_curve_add",
                        text="Bezier", icon='CURVE_BEZCURVE')
        layout.operator("curve.primitive_bezier_circle_add",
                        text="Circle", icon='CURVE_BEZCIRCLE')

        layout.separator()

        layout.operator("curve.primitive_nurbs_curve_add",
                        text="Nurbs Curve", icon='CURVE_NCURVE')
        layout.operator("curve.primitive_nurbs_circle_add",
                        text="Nurbs Circle", icon='CURVE_NCIRCLE')
        layout.operator("curve.primitive_nurbs_path_add",
                        text="Path", icon='CURVE_PATH')

        layout.separator()

        layout.operator("object.curves_empty_hair_add",
                        text="Empty Hair", icon='CURVES_DATA')
        layout.operator("object.quick_fur", text="Fur", icon='CURVES_DATA')

        experimental = context.preferences.experimental
        if experimental.use_new_curves_tools:
            layout.operator("object.curves_random_add",
                            text="Random", icon='CURVES_DATA')
...
class VIEW3D_MT_surface_add(Menu):
    ...
    def draw(self, _context):
        ...
        layout.operator("surface.primitive_nurbs_surface_curve_add",
                        text="Nurbs Curve", icon='SURFACE_NCURVE')
        layout.operator("surface.primitive_nurbs_surface_circle_add",
                        text="Nurbs Circle", icon='SURFACE_NCIRCLE')
        layout.operator("surface.primitive_nurbs_surface_surface_add",
                        text="Nurbs Surface", icon='SURFACE_NSURFACE')
        ...
        layout.operator("surface.primitive_nurbs_surface_sphere_add",
                        text="Nurbs Sphere", icon='SURFACE_NSPHERE')
        layout.operator("surface.primitive_nurbs_surface_torus_add",
                        text="Nurbs Torus", icon='SURFACE_NTORUS')
...
class TOPBAR_MT_edit_armature_add(Menu):
    ...
    def draw(self, _context):
        ...
        layout.operator("armature.bone_primitive_add",
                        text="Single Bone", icon='BONE_DATA')
...
class VIEW3D_MT_volume_add(Menu):
    ...
    def draw(self, _context):
        layout = self.layout
        layout.operator("object.volume_import",
                        text="Import OpenVDB...", icon='OUTLINER_DATA_VOLUME')
...
class VIEW3D_MT_add(Menu):
    ...
    def draw(self, context):
        ...
        layout.menu("VIEW3D_MT_metaball_add",
                    text="Metaball", icon='OUTLINER_OB_META')
        layout.operator("object.text_add", text="Text",
                        icon='OUTLINER_OB_FONT')
        if context.preferences.experimental.use_new_point_cloud_type:
            layout.operator("object.pointcloud_add",
                            text="Point Cloud", icon='OUTLINER_OB_POINTCLOUD')
        layout.menu("VIEW3D_MT_volume_add", text="Volume",
                    text_ctxt=i18n_contexts.id_id, icon='OUTLINER_OB_VOLUME')
        layout.operator_menu_enum(
            "object.gpencil_add", "type", text="Grease Pencil", icon='OUTLINER_OB_GREASEPENCIL')
        ...
        if VIEW3D_MT_armature_add.is_extended():
            ...
            layout.operator("object.armature_add",
                            text="Armature", icon='OUTLINER_OB_ARMATURE')
        ...
        layout.menu("VIEW3D_MT_image_add", text="Image",
        ...
        layout.operator_menu_enum(
            "object.effector_add", "type", text="Force Field", icon='OUTLINER_OB_FORCE_FIELD')
...
class VIEW3D_MT_image_add(Menu):
    ...
    def draw(self, _context):
        layout = self.layout
        layout.operator("object.load_reference_image",
                        text="Reference", icon='IMAGE_REFERENCE')
        layout.operator("object.load_background_image",
...
class VIEW3D_MT_object_relations(Menu):
    ...
    def draw(self, _context):
        ...
        layout.operator_menu_enum(
            "object.make_local", "type", text="Make Local...")
...
class VIEW3D_MT_object(Menu):
    ...
    def draw(self, _context):
        ...
        layout.operator("view3d.copybuffer",
                        text="Copy Objects", icon='COPYDOWN')
        layout.operator("view3d.pastebuffer",
                        text="Paste Objects", icon='PASTEDOWN')
        ...
        layout.operator("object.shade_smooth",
                        text="Shade Auto Smooth").use_auto_smooth = True
        ...
        layout.operator("object.delete",
                        text="Delete Global").use_global = True
...
class VIEW3D_MT_object_animation(Menu):
    ...
    def draw(self, _context):
        ...
        layout.operator("gpencil.bake_mesh_animation",
                        text="Bake Mesh to Grease Pencil...")
        layout.operator("gpencil.bake_grease_pencil_animation",
...
class VIEW3D_MT_object_rigid_body(Menu):
    ...
    def draw(self, _context):
        ...
        layout.operator("rigidbody.objects_add",
                        text="Add Active").type = 'ACTIVE'
        layout.operator("rigidbody.objects_add",
                        text="Add Passive").type = 'PASSIVE'
...
class VIEW3D_MT_object_clear(Menu):
    ...
    def draw(self, _context):
        ...
        layout.operator("object.location_clear",
                        text="Location").clear_delta = False
        layout.operator("object.rotation_clear",
                        text="Rotation").clear_delta = False
...
class VIEW3D_MT_object_context_menu(Menu):
    ...
    def draw(self, context):
        ...
        if obj is None:
            ...
            props = layout.operator(
                "wm.context_modal_mouse", text="Adjust Extrusion")
            props.data_path_iter = "selected_editable_objects"
            ...
            props = layout.operator(
                "wm.context_modal_mouse", text="Adjust Offset")
            props.data_path_iter = "selected_editable_objects"
            ...
            props = layout.operator(
                "wm.context_modal_mouse", text="Adjust Light Power")
            props.data_path_iter = "selected_editable_objects"
            ...
            if light.type == 'SPOT':
                ...
                props = layout.operator(
                    "wm.context_modal_mouse", text="Adjust Spot Light Size")
                props.data_path_iter = "selected_editable_objects"
        ...
        if obj is not None:
            ...
            if obj.type == 'GPENCIL':
                layout.operator_menu_enum(
                    "gpencil.convert", "type", text="Convert To")
        ...
        props = layout.operator(
            "wm.call_panel", text="Rename Active Object...")
...
class VIEW3D_MT_object_apply(Menu):
    ...
    def draw(self, _context):
        ...
        props = layout.operator("object.transform_apply",
                                text="Location", text_ctxt=i18n_contexts.default)
        props.location, props.rotation, props.scale = True, False, False

        props = layout.operator("object.transform_apply",
                                text="Rotation", text_ctxt=i18n_contexts.default)
        props.location, props.rotation, props.scale = False, True, False

        props = layout.operator("object.transform_apply",
                                text="Scale", text_ctxt=i18n_contexts.default)
        props.location, props.rotation, props.scale = False, False, True

        props = layout.operator(
            "object.transform_apply", text="All Transforms", text_ctxt=i18n_contexts.default)
        props.location, props.rotation, props.scale = True, True, True

        props = layout.operator(
            "object.transform_apply", text="Rotation & Scale", text_ctxt=i18n_contexts.default)
...
class VIEW3D_MT_object_parent(Menu):
    ...
    def draw(self, _context):
        ...
        with operator_context(layout, 'EXEC_REGION_WIN'):
            layout.operator(
                "object.parent_no_inverse_set").keep_transform = False
            props = layout.operator(
                "object.parent_no_inverse_set", text="Make Parent without Inverse (Keep Transform)")
...
class VIEW3D_MT_object_showhide(Menu):
    ...
    def draw(self, _context):
        ...
        layout.operator("object.hide_view_set",
                        text="Hide Selected").unselected = False
        layout.operator("object.hide_view_set",
                        text="Hide Unselected").unselected = True
...
class VIEW3D_MT_object_cleanup(Menu):
    ...
    def draw(self, _context):
        ...
        layout.operator("object.vertex_group_clean",
                        text="Clean Vertex Group Weights").group_select_mode = 'ALL'
        layout.operator("object.vertex_group_limit_total",
                        text="Limit Total Vertex Groups").group_select_mode = 'ALL'
...
class VIEW3D_MT_object_asset(Menu):
    ...
    def draw(self, _context):
        ...
        layout.operator(
            "asset.clear", text="Clear Asset").set_fake_user = False
        layout.operator(
            "asset.clear", text="Clear Asset (Set Fake User)").set_fake_user = True
...
class VIEW3D_MT_make_single_user(Menu):
    ...
    def draw(self, _context):
        ...
        props = layout.operator(
            "object.make_single_user", text="Object & Data")
        props.object = props.obdata = True
        props.material = props.animation = props.obdata_animation = False

        props = layout.operator("object.make_single_user",
        ...
        props = layout.operator(
            "object.make_single_user", text="Object Animation")
        props.animation = True
...
class VIEW3D_MT_object_convert(Menu):
    ...
    def draw(self, context):
        ...
        if ob and ob.type == 'CURVES':
            layout.operator("curves.convert_to_particle_system",
...
class VIEW3D_MT_make_links(Menu):
    ...
    def draw(self, _context):
        ...
        if len(bpy.data.scenes) > 10:
            layout.operator_context = 'INVOKE_REGION_WIN'
            layout.operator("object.make_links_scene",
                            text="Link Objects to Scene...", icon='OUTLINER_OB_EMPTY')
        else:
            layout.operator_context = 'EXEC_REGION_WIN'
            layout.operator_menu_enum(
...
class VIEW3D_MT_hook(Menu):
    ...
    def draw(self, context):
        ...
        layout.operator("object.hook_add_selob",
                        text="Hook to Selected Object Bone").use_bone = True
    ...
    def draw(self, context):
        ...
        if ob.mode == 'EDIT' or (ob.mode == 'WEIGHT_PAINT' and ob.type == 'MESH' and ob.data.use_paint_mask_vertex):
            if ob.vertex_groups.active:
                ...
                layout.operator("object.vertex_group_assign",
                ...
                layout.operator("object.vertex_group_remove_from",
                                text="Remove from All").use_all_groups = True
        ...
        if ob.vertex_groups.active:
            ...
            layout.operator("object.vertex_group_remove",
                            text="Remove Active Group").all = False
            layout.operator("object.vertex_group_remove",
                            text="Remove All Groups").all = True
...
class VIEW3D_MT_paint_weight_lock(Menu):
    ...
    def draw(self, _context):
        ...
        props = layout.operator(
            "object.vertex_group_lock", text="Lock Selected")
        props.action, props.mask = 'LOCK', 'SELECTED'

        props = layout.operator(
            "object.vertex_group_lock", text="Lock Unselected")
        props.action, props.mask = 'LOCK', 'UNSELECTED'

        props = layout.operator(
            "object.vertex_group_lock", text="Lock Only Selected")
        props.action, props.mask = 'LOCK', 'INVERT_UNSELECTED'

        props = layout.operator(
            "object.vertex_group_lock", text="Lock Only Unselected")
        ...
        props = layout.operator(
            "object.vertex_group_lock", text="Unlock Selected")
        props.action, props.mask = 'UNLOCK', 'SELECTED'

        props = layout.operator(
            "object.vertex_group_lock", text="Unlock Unselected")
...
class VIEW3D_MT_paint_weight(Menu):
    ...
    def draw_generic(layout, is_editmode=False):
        ...
        layout.operator("object.vertex_group_normalize_all",
                        text="Normalize All")
        ...
        if not is_editmode:
            props = layout.operator(
                "object.data_transfer", text="Transfer Weights")
...
class VIEW3D_MT_sculpt(Menu):
    ...
    def draw(self, _context):
        ...
        layout.operator("sculpt.face_set_invert_visibility",
                        text="Invert Visible")
...
class VIEW3D_MT_sculpt_curves(Menu):
    ...
    def draw(self, _context):
        ...
        layout.operator("curves.snap_curves_to_surface",
                        text="Snap to Deformed Surface").attach_mode = 'DEFORM'
        layout.operator("curves.snap_curves_to_surface",
                        text="Snap to Nearest Surface").attach_mode = 'NEAREST'
        layout.separator()
        layout.operator("curves.convert_to_particle_system",
...
class VIEW3D_MT_mask(Menu):
    ...
    def draw(self, _context):
        ...
        props = layout.operator(
            "sculpt.expand", text="Expand Mask by Topology")
        props.target = 'MASK'
        ...
        props = layout.operator("mesh.paint_mask_slice",
        ...
        props = layout.operator(
            "sculpt.mask_from_cavity", text="Mask From Cavity")
...
class VIEW3D_MT_face_sets(Menu):
    ...
    def draw(self, _context):
        ...
        props = layout.operator("sculpt.face_sets_create",
                                text="Face Set from Masked")
        props.mode = 'MASKED'

        props = layout.operator("sculpt.face_sets_create",
                                text="Face Set from Visible")
        props.mode = 'VISIBLE'
        ...
        props = layout.operator(
            "sculpt.expand", text="Expand Face Set by Topology")
        ...
        props = layout.operator("mesh.face_set_extract",
                                text="Extract Face Set")
...
class VIEW3D_MT_sculpt_set_pivot(Menu):
    ...
    def draw(self, _context):
        ...
        props = layout.operator(
            "sculpt.set_pivot_position", text="Pivot to Origin")
        props.mode = 'ORIGIN'

        props = layout.operator(
            "sculpt.set_pivot_position", text="Pivot to Unmasked")
        props.mode = 'UNMASKED'
...
class VIEW3D_MT_particle_context_menu(Menu):
    ...
    def draw(self, context):
        ...
        if particle_edit.select_mode == 'POINT':
            ...
            layout.operator("particle.select_all",
                            text="All").action = 'SELECT'
            layout.operator("particle.select_all",
                            text="None").action = 'DESELECT'
            layout.operator("particle.select_all",
                            text="Invert").action = 'INVERT'
...
class VIEW3D_MT_pose(Menu):
    ...
    def draw(self, _context):
        ...
        layout.operator("pose.paste", icon='PASTEFLIPDOWN',
                        text="Paste Pose Flipped").flipped = True
...
class VIEW3D_MT_pose_propagate(Menu):
    ...
    def draw(self, _context):
        ...
        layout.operator("pose.propagate",
                        text="To Next Keyframe").mode = 'NEXT_KEY'
        layout.operator(
            "pose.propagate", text="To Last Keyframe (Make Cyclic)").mode = 'LAST_KEY'

        layout.separator()

        layout.operator("pose.propagate",
                        text="On Selected Keyframes").mode = 'SELECTED_KEYS'

        layout.separator()

        layout.operator("pose.propagate",
                        text="On Selected Markers").mode = 'SELECTED_MARKERS'
...
class VIEW3D_MT_pose_group(Menu):
    ...
    def draw(self, context):
        ...
        layout.operator("pose.group_assign",
                        text="Assign to New Group").type = 0

        if pose.bone_groups:
            active_group = pose.bone_groups.active_index + 1
            layout.operator("pose.group_assign",
                            text="Assign to Group").type = active_group
...
class VIEW3D_MT_pose_constraints(Menu):
    ...
    def draw(self, _context):
        ...
        layout.operator("pose.constraint_add_with_targets",
...
class VIEW3D_MT_pose_names(Menu):
    ...
    def draw(self, _context):
        ...
        layout.operator("pose.autoside_names",
                        text="Auto-Name Left/Right").axis = 'XAXIS'
        layout.operator("pose.autoside_names",
                        text="Auto-Name Front/Back").axis = 'YAXIS'
        layout.operator("pose.autoside_names",
                        text="Auto-Name Top/Bottom").axis = 'ZAXIS'
...
class VIEW3D_MT_pose_apply(Menu):
    ...
    def draw(self, _context):
        ...
        layout.operator("pose.armature_apply",
                        text="Apply Selected as Rest Pose").selected = True
...
class VIEW3D_MT_edit_mesh(Menu):
    ...
    def draw(self, _context):
        ...
        layout.operator_menu_enum(
            "mesh.sort_elements", "type", text="Sort Elements...")
...
class VIEW3D_MT_edit_mesh_context_menu(Menu):
    ...
    def draw(self, context):
        ...
        if is_vert_mode:
            ...
            col.operator(
                "mesh.bevel", text="Bevel Vertices").affect = 'VERTICES'

            if selected_verts_len > 1:
                col.separator()
                col.operator("mesh.edge_face_add",
                             text="New Edge/Face from Vertices")
            ...
            col.operator("transform.vertex_random",
                         text="Randomize Vertices").offset = 0.1
            col.operator("mesh.vertices_smooth",
                         text="Smooth Vertices").factor = 0.5
            col.operator_context = 'INVOKE_REGION_WIN'
        ...
        if is_edge_mode:
            ...
            col.operator("mesh.edge_rotate",
                         text="Rotate Edge CW").use_ccw = False
            ...
            if with_freestyle:
                ...
                col.operator("mesh.mark_freestyle_edge",
                             text="Clear Freestyle Edge").clear = True
...
class VIEW3D_MT_edit_mesh_select_mode(Menu):
    ...
    def draw(self, _context):
        ...
        layout.operator("mesh.select_mode", text="Edge",
                        icon='EDGESEL').type = 'EDGE'
        layout.operator("mesh.select_mode", text="Face",
...
class VIEW3D_MT_edit_mesh_extrude(Menu):
    ...
    _extrude_funcs = {
        ...
        layout.operator("view3d.edit_mesh_extrude_move_shrink_fatten",
                        text="Extrude Faces Along Normals"),
        'FACE': lambda layout:
        layout.operator("mesh.extrude_faces_move",
                        text="Extrude Individual Faces"),
...
class VIEW3D_MT_edit_mesh_vertices(Menu):
    ...
    def draw(self, _context):
        ...
        layout.operator("mesh.vertices_smooth",
                        text="Smooth Vertices").factor = 0.5
        layout.operator("mesh.vertices_smooth_laplacian",
                        text="Smooth Vertices (Laplacian)")
        layout.operator_context = 'INVOKE_REGION_WIN'
...
class VIEW3D_MT_edit_mesh_edges(Menu):
    ...
    def draw(self, _context):
        ...
        layout.operator("mesh.edge_rotate",
                        text="Rotate Edge CW").use_ccw = False
        layout.operator("mesh.edge_rotate",
                        text="Rotate Edge CCW").use_ccw = True
        ...
        layout.operator("mesh.mark_sharp",
                        text="Mark Sharp from Vertices").use_verts = True
        props = layout.operator(
            "mesh.mark_sharp", text="Clear Sharp from Vertices")
    ...
    def draw(self, _context):
        ...
        if with_freestyle:
            ...
            layout.operator("mesh.mark_freestyle_face",
                            text="Clear Freestyle Face").clear = True
...
class VIEW3D_MT_edit_mesh_normals_average(Menu):
    ...
    def draw(self, _context):
        ...
        layout.operator("mesh.average_normals",
                        text="Custom Normal").average_type = 'CUSTOM_NORMAL'
        layout.operator("mesh.average_normals",
                        text="Face Area").average_type = 'FACE_AREA'
        layout.operator("mesh.average_normals",
                        text="Corner Angle").average_type = 'CORNER_ANGLE'
...
class VIEW3D_MT_edit_mesh_normals(Menu):
    ...
    def draw(self, _context):
        ...
        layout.operator("mesh.normals_tools",
                        text="Paste Vector").mode = 'PASTE'

        layout.operator("mesh.smooth_normals", text="Smooth Vectors")
        layout.operator("mesh.normals_tools",
                        text="Reset Vectors").mode = 'RESET'
...
class VIEW3D_MT_edit_mesh_shading(Menu):
    ...
    def draw(self, _context):
        ...
        layout.operator("mesh.mark_sharp",
                        text="Sharp Vertices").use_verts = True
...
class VIEW3D_MT_edit_gpencil_delete(Menu):
    ...
    def draw(self, _context):
        ...
        layout.operator(
            "gpencil.delete", text="Delete Active Keyframe (Active Layer)").type = 'FRAME'
...
class VIEW3D_MT_edit_font_chars(Menu):
    ...
    def draw(self, _context):
        ...
        layout.operator("font.text_insert",
        ...
        layout.operator("font.text_insert",
                        text="Multiplication Sign").text = "\u00D7"
        layout.operator("font.text_insert", text="Circle").text = "\u008A"

        layout.separator()

        layout.operator("font.text_insert",
                        text="Superscript 1").text = "\u00B9"
        layout.operator("font.text_insert",
                        text="Superscript 2").text = "\u00B2"
        layout.operator("font.text_insert",
                        text="Superscript 3").text = "\u00B3"
        ...
        layout.operator("font.text_insert",
                        text="Dutch Florin").text = "\u00A4"
        layout.operator("font.text_insert",
                        text="British Pound").text = "\u00A3"
        layout.operator("font.text_insert",
                        text="Japanese Yen").text = "\u00A5"

        layout.separator()

        layout.operator("font.text_insert", text="German S").text = "\u00DF"
        layout.operator("font.text_insert",
                        text="Spanish Question Mark").text = "\u00BF"
        layout.operator("font.text_insert",
                        text="Spanish Exclamation Mark").text = "\u00A1"
...
class VIEW3D_MT_edit_font_kerning(Menu):
    ...
    def draw(self, context):
        ...
        layout.operator("font.change_spacing",
                        text="Decrease Kerning").delta = -1.0
        layout.operator("font.change_spacing",
                        text="Increase Kerning").delta = 1.0
        layout.operator("font.change_spacing",
                        text="Reset Kerning").delta = -kerning
...
class VIEW3D_MT_edit_font_delete(Menu):
    ...
    def draw(self, _context):
        ...
        layout.operator(
            "font.delete", text="Previous Character").type = 'PREVIOUS_CHARACTER'
        layout.operator(
            "font.delete", text="Next Character").type = 'NEXT_CHARACTER'
        layout.operator(
            "font.delete", text="Previous Word").type = 'PREVIOUS_WORD'
...
class VIEW3D_MT_edit_font(Menu):
    ...
    def draw(self, _context):
        ...
        layout.operator("font.style_toggle", text="Toggle Bold",
                        icon='BOLD').style = 'BOLD'
        layout.operator("font.style_toggle", text="Toggle Italic",
...
class VIEW3D_MT_edit_meta_showhide(Menu):
    ...
    def draw(self, _context):
        ...
        layout.operator("mball.hide_metaelems",
                        text="Hide Selected").unselected = False
        layout.operator("mball.hide_metaelems",
                        text="Hide Unselected").unselected = True
...
class VIEW3D_MT_edit_armature_names(Menu):
    ...
    def draw(self, _context):
        ...
        layout.operator("armature.autoside_names",
                        text="Auto-Name Left/Right").type = 'XAXIS'
        layout.operator("armature.autoside_names",
                        text="Auto-Name Front/Back").type = 'YAXIS'
        layout.operator("armature.autoside_names",
                        text="Auto-Name Top/Bottom").type = 'ZAXIS'
...
class VIEW3D_MT_gpencil_autoweights(Menu):
    ...
    def draw(self, _context):
        layout = self.layout
        layout.operator("gpencil.generate_weights",
                        text="With Empty Groups").mode = 'NAME'
        layout.operator("gpencil.generate_weights",
                        text="With Automatic Weights").mode = 'AUTO'
...
class VIEW3D_MT_draw_gpencil(Menu):
    ...
    def draw(self, _context):
        ...
        layout.operator("gpencil.interpolate_sequence",
...
class VIEW3D_MT_edit_gpencil(Menu):
    ...
    def draw(self, _context):
        ...
        layout.operator_menu_enum(
            "gpencil.stroke_separate", "mode", text="Separate")
        layout.menu("GPENCIL_MT_cleanup")
...
class VIEW3D_MT_edit_gpencil_stroke(Menu):
    ...
    def draw(self, context):
        ...
        layout.operator_menu_enum(
            "gpencil.stroke_arrange", "direction", text="Arrange")
        ...
        props.type = 'CLOSE'
        props.geometry = True
        layout.operator("gpencil.stroke_cyclical_set",
                        text="Toggle Cyclic").type = 'TOGGLE'
        layout.operator_menu_enum(
            "gpencil.stroke_caps_set", text="Toggle Caps", property="type")
        layout.operator("gpencil.stroke_flip", text="Switch Direction")
        layout.operator("gpencil.stroke_start_set", text="Set Start Point")

        layout.separator()
        layout.operator("gpencil.stroke_normalize",
                        text="Normalize Thickness").mode = 'THICKNESS'
        layout.operator("gpencil.stroke_normalize",
                        text="Normalize Opacity").mode = 'OPACITY'
        layout.prop(settings, "use_scale_thickness", text="Scale Thickness")

        layout.separator()
        layout.operator("gpencil.reset_transform_fill",
                        text="Reset Fill Transform")
...
class VIEW3D_MT_edit_gpencil_point(Menu):
    ...
    def draw(self, _context):
        ...
        layout.operator("gpencil.stroke_smooth",
                        text="Smooth").only_selected = True
...
class VIEW3D_MT_weight_gpencil(Menu):
    ...
    def draw(self, _context):
        ...
        layout.operator("gpencil.vertex_group_normalize_all",
                        text="Normalize All")
...
class VIEW3D_MT_gpencil_animation(Menu):
    ...
    def draw(self, _context):
        ...
        layout.operator("gpencil.blank_frame_add",
                        text="Insert Blank Keyframe (All Layers)").all_layers = True

        layout.separator()

        layout.operator("gpencil.frame_duplicate",
                        text="Duplicate Active Keyframe (Active Layer)")
        layout.operator("gpencil.frame_duplicate",
                        text="Duplicate Active Keyframe (All Layers)").mode = 'ALL'
...
class VIEW3D_MT_edit_gpencil_transform(Menu):
    ...
    def draw(self, _context):
        ...
        layout.operator("transform.tosphere", text="To Sphere")
        layout.operator("transform.transform",
                        text="Shrink/Fatten").mode = 'GPENCIL_SHRINKFATTEN'
...
class VIEW3D_MT_edit_gpencil_showhide(Menu):
    ...
    def draw(self, _context):
        ...
        layout.operator(
            "gpencil.hide", text="Hide Active Layer").unselected = False
        layout.operator(
            "gpencil.hide", text="Hide Inactive Layers").unselected = True
...
class VIEW3D_MT_view_pie(Menu):
    ...
    def draw(self, _context):
        ...
        pie.operator("view3d.view_camera",
                     text="View Camera", icon='CAMERA_DATA')
        pie.operator("view3d.view_selected",
                     text="View Selected", icon='ZOOM_SELECTED')
...
class VIEW3D_MT_transform_gizmo_pie(Menu):
    ...
    def draw(self, context):
        ...
        pie = layout.menu_pie()
        # 1: Left
        pie.operator("view3d.transform_gizmo_set",
                     text="Move").type = {'TRANSLATE'}
        # 2: Right
        pie.operator("view3d.transform_gizmo_set",
                     text="Rotate").type = {'ROTATE'}
        # 3: Down
        pie.operator("view3d.transform_gizmo_set",
                     text="Scale").type = {'SCALE'}
        # 4: Up
        pie.prop(context.space_data, "show_gizmo",
                 text="Show Gizmos", icon='GIZMO')
        # 5: Up/Left
...
class VIEW3D_MT_shading_ex_pie(Menu):
    ...
    def draw(self, context):
        ...
        if context.pose_object:
            ...
            sub.prop(
                view.shading,
                "show_xray_wireframe" if (
                    view.shading.type == 'WIREFRAME') else "show_xray",
                text="Toggle X-Ray",
                icon='XRAY',
            )

        pie.prop(view.overlay, "show_overlays",
                 text="Toggle Overlays", icon='OVERLAY')
...
class VIEW3D_MT_pivot_pie(Menu):
    ...
    def draw(self, context):
        ...
        pie.prop_enum(context.scene.tool_settings,
                      "transform_pivot_point", value='BOUNDING_BOX_CENTER')
        pie.prop_enum(context.scene.tool_settings,
                      "transform_pivot_point", value='CURSOR')
        pie.prop_enum(context.scene.tool_settings,
                      "transform_pivot_point", value='INDIVIDUAL_ORIGINS')
        pie.prop_enum(context.scene.tool_settings,
                      "transform_pivot_point", value='MEDIAN_POINT')
        pie.prop_enum(context.scene.tool_settings,
                      "transform_pivot_point", value='ACTIVE_ELEMENT')
...
class VIEW3D_MT_snap_pie(Menu):
    ...
    def draw(self, _context):
        ...
        pie.operator("view3d.snap_cursor_to_grid",
                     text="Cursor to Grid", icon='CURSOR')
        pie.operator("view3d.snap_selected_to_grid",
                     text="Selection to Grid", icon='RESTRICT_SELECT_OFF')
        pie.operator("view3d.snap_cursor_to_selected",
                     text="Cursor to Selected", icon='CURSOR')
        pie.operator(
        ...
        pie.operator("view3d.snap_selected_to_active",
                     text="Selection to Active", icon='RESTRICT_SELECT_OFF')
        ...
        pie.operator("view3d.snap_cursor_to_active",
                     text="Cursor to Active", icon='CURSOR')
...
class VIEW3D_MT_sculpt_automasking_pie(Menu):
    ...
    def draw(self, context):
        ...
        pie.prop(sculpt, "use_automasking_boundary_face_sets",
                 text="Face Sets Boundary")
        pie.prop(sculpt, "use_automasking_cavity", text="Cavity")
        pie.prop(sculpt, "use_automasking_cavity_inverted",
                 text="Cavity (Inverted)")
...
class VIEW3D_MT_sculpt_gpencil_automasking_pie(Menu):
    ...
    def draw(self, context):
        ...
        pie.prop(tool_settings.gpencil_sculpt,
                 "use_automasking_stroke", text="Stroke")
        pie.prop(tool_settings.gpencil_sculpt,
                 "use_automasking_layer_stroke", text="Layer")
        pie.prop(tool_settings.gpencil_sculpt,
                 "use_automasking_material_stroke", text="Material")
        pie.prop(tool_settings.gpencil_sculpt,
                 "use_automasking_layer_active", text="Active Layer")
        pie.prop(tool_settings.gpencil_sculpt,
                 "use_automasking_material_active", text="Active Material")
...
class VIEW3D_MT_sculpt_face_sets_edit_pie(Menu):
    ...
    def draw(self, _context):
        ...
        props = pie.operator("sculpt.face_sets_create",
                             text="Face Set from Masked")
        props.mode = 'MASKED'
        ...
        pie.operator("sculpt.face_set_invert_visibility",
                     text="Invert Visible")
...
class VIEW3D_MT_wpaint_vgroup_lock_pie(Menu):
    ...
    def draw(self, _context):
        ...
        # 1: Left
        props = pie.operator("object.vertex_group_lock",
        ...
        # 3: Down
        props = pie.operator("object.vertex_group_lock",
                             icon='UNLOCKED', text="Unlock Selected")
        props.action, props.mask = 'UNLOCK', 'SELECTED'
        # 4: Up
        props = pie.operator("object.vertex_group_lock",
                             icon='LOCKED', text="Lock Selected")
        props.action, props.mask = 'LOCK', 'SELECTED'
        # 5: Up/Left
        props = pie.operator("object.vertex_group_lock",
                             icon='LOCKED', text="Lock Unselected")
        props.action, props.mask = 'LOCK', 'UNSELECTED'
        # 6: Up/Right
        props = pie.operator("object.vertex_group_lock",
...
class VIEW3D_PT_view3d_properties(Panel):
    ...
    def draw(self, context):
        ...
        subcol = col.column()
        subcol.active = bool(view.region_3d.view_perspective !=
                             'CAMERA' or view.region_quadviews)
...
class VIEW3D_PT_view3d_lock(Panel):
    ...
    def draw(self, context):
        ...
        col = layout.column(align=True)
        sub = col.column()
        sub.active = bool(view.region_3d.view_perspective !=
                          'CAMERA' or view.region_quadviews)
...
class VIEW3D_PT_collections(Panel):
    ...
    def _draw_collection(self, layout, view_layer, use_local_collections, collection, index):
        ...
        for child in collection.children:
            ...
            if not use_local_collections:
                ...
                subrow.active = collection.visible_get()
    ...
    def draw(self, context):
        ...
        self._draw_collection(
            layout, view_layer, view.use_local_collections, view_layer.layer_collection, 0)
    ...
    def draw_ex(self, _context, view, show_select):
        ...
        for attr, attr_name in attr_object_types:
            ...
            if show_select:
                ...
                icon_s = 'RESTRICT_SELECT_OFF' if getattr(
                    view, attr_s) else 'RESTRICT_SELECT_ON'
    ...
    def draw(self, context):
        ...
        if shading.type == 'SOLID':
            ...
            if not shading.use_scene_world:
                ...
                row = col.row()
                row.prop(shading, "use_studiolight_view_rotation",
                         text="", icon='WORLD', toggle=True)
                row = row.row()
                row.prop(shading, "studiolight_rotate_z", text="Rotation")
...
class VIEW3D_PT_shading_compositor(Panel):
    ...
    def draw(self, context):
        ...
        if shading.use_compositor != "DISABLED" and not is_supported:
            self.layout.label(
                text="Compositor not supported on this platform", icon='ERROR')
...
class VIEW3D_PT_overlay_geometry(Panel):
    ...
    def draw(self, context):
        ...
        if context.mode != 'OBJECT':
            ...
            sub.prop(overlay, "fade_inactive_alpha",
                     text="Fade Inactive Geometry")
...
class VIEW3D_PT_overlay_edit_mesh(Panel):
    ...
    def draw(self, context):
        ...
        row.prop(overlay, "show_edge_sharp", text="Sharp",
                 text_ctxt=i18n_contexts.plural, toggle=True)
...
class VIEW3D_PT_overlay_edit_mesh_normals(Panel):
    ...
    def draw(self, context):
        ...
        row = col.row(align=True)
        row.prop(overlay, "show_vertex_normals",
                 text="", icon='NORMALS_VERTEX')
        row.prop(overlay, "show_split_normals",
                 text="", icon='NORMALS_VERTEX_FACE')
...
class VIEW3D_PT_transform_orientations(Panel):
    ...
    def draw(self, context):
        ...
        row.operator("transform.create_orientation", text="",
                     icon='ADD', emboss=False).use = True

        if orientation:
            row = layout.row(align=False)
            row.prop(orientation, "name", text="", icon='OBJECT_ORIGIN')
            row.operator("transform.delete_orientation",
...
class VIEW3D_PT_overlay_gpencil_options(Panel):
    ...
    def draw(self, context):
        ...
        sub.prop(overlay, "gpencil_fade_layer",
                 text="Fade Inactive Layers", slider=True)

        row = col.row()
        row.prop(overlay, "use_gpencil_fade_objects", text="")
        sub = row.row(align=True)
        sub.active = overlay.use_gpencil_fade_objects
        sub.prop(overlay, "gpencil_fade_objects",
                 text="Fade Inactive Objects", slider=True)
        sub.prop(overlay, "use_gpencil_fade_gp_objects",
                 text="", icon='OUTLINER_OB_GREASEPENCIL')
        ...
        if context.object.mode in {'EDIT_GPENCIL', 'SCULPT_GPENCIL', 'WEIGHT_GPENCIL', 'VERTEX_GPENCIL'}:
            ...
            col = split.column()
            col.prop(overlay, "use_gpencil_multiedit_line_only",
                     text="Only in Multiframe")
            ...
            if context.object.mode == 'EDIT_GPENCIL':
                ...
                col = split.column()
                col.prop(overlay, "use_gpencil_show_material_name",
                         text="Material Name")

                if not gpd.use_curve_edit:
                    layout.prop(overlay, "vertex_opacity",
                                text="Vertex Opacity", slider=True)
                else:
        ...
        if context.object.mode in {'PAINT_GPENCIL', 'VERTEX_GPENCIL'}:
            ...
            row.prop(overlay, "gpencil_vertex_paint_opacity",
...
class VIEW3D_PT_context_properties(Panel):
    ...
    def draw(self, context):
        ...
        if member:
            ...
            rna_prop_ui.draw(self.layout, context, member,
                             object, use_edit=False)
...
class VIEW3D_MT_gpencil_edit_context_menu(Menu):
    ...
    def draw(self, context):
        ...
        if is_point_mode or is_segment_mode:
            ...
            # Additive Operators
            col.operator("gpencil.stroke_subdivide",
                         text="Subdivide").only_selected = True
            ...
            # Deform Operators
            col.operator("gpencil.stroke_smooth",
                         text="Smooth").only_selected = True
            col.operator("transform.bend", text="Bend")
            col.operator("transform.shear", text="Shear")
            col.operator("transform.tosphere", text="To Sphere")
            col.operator("transform.transform",
                         text="Shrink/Fatten").mode = 'GPENCIL_SHRINKFATTEN'
            ...
            col.operator("gpencil.paste", text="Paste",
                         icon='PASTEDOWN').type = 'ACTIVE'
            ...
            col.operator(
                "gpencil.stroke_merge_by_distance").use_unselected = False
            col.operator("gpencil.stroke_split", text="Split")
            col.operator("gpencil.stroke_separate",
                         text="Separate").mode = 'POINT'

            col.separator()

            col.operator("gpencil.delete", text="Delete").type = 'POINTS'
            col.operator("gpencil.dissolve", text="Dissolve").type = 'POINTS'
            col.operator("gpencil.dissolve",
                         text="Dissolve Between").type = 'BETWEEN'
            col.operator("gpencil.dissolve",
                         text="Dissolve Unselected").type = 'UNSELECT'
        ...
        if is_stroke_mode:
            ...
            col.operator("gpencil.stroke_smooth",
                         text="Smooth").only_selected = False
            col.operator("transform.transform",
                         text="Shrink/Fatten").mode = 'GPENCIL_SHRINKFATTEN'
            ...
            col.operator("gpencil.set_active_material",
                         text="Set as Active Material")
            col.operator_menu_enum(
                "gpencil.stroke_arrange", "direction", text="Arrange")
            ...
            # Removal Operators
            col.operator(
                "gpencil.stroke_merge_by_distance").use_unselected = True
            col.operator_menu_enum("gpencil.stroke_join", "type", text="Join")
            col.operator("gpencil.stroke_split", text="Split")
            col.operator("gpencil.stroke_separate",
                         text="Separate").mode = 'STROKE'
...
def draw_gpencil_layer_active(context, layout):
    ...
    if gpl:
        ...
        row.operator_menu_enum("gpencil.layer_change",
                               "layer", text="", icon='GREASEPENCIL')
    ...
    if ob and len(ob.material_slots) > 0 and ob.active_material_index >= 0:
        ...
        if ma:
            ...
            row.operator_menu_enum(
                "gpencil.material_set", "slot", text="", icon='MATERIAL')
...
class VIEW3D_PT_gpencil_vertex_context_menu(Panel):
    ...
    def draw(self, context):
        ...
        if brush.gpencil_vertex_tool in {'DRAW', 'BLUR', 'SMEAR'}:
            ...
            row.prop(gp_settings, "use_strength_pressure",
...
class VIEW3D_PT_sculpt_automasking(Panel):
    ...
    def draw(self, context):
        ...
        col.prop(sculpt, "use_automasking_cavity_inverted",
                 text="Cavity (inverted)")
        ...
        if is_cavity_active:
            ...
            col = layout.column()
            col.prop(sculpt, "use_automasking_custom_cavity_curve",
                     text="Custom Curve")
        ...
        if sculpt.use_automasking_view_normal:
            ...
            subcol.prop(sculpt, "automasking_view_normal_falloff",
                        text="Falloff")
...
class VIEW3D_PT_curves_sculpt_add_shape(Panel):
    ...
    def draw(self, context):
        ...
        col.prop(brush.curves_sculpt_settings,
                 "interpolate_length", text="Length")
        col.prop(brush.curves_sculpt_settings,
                 "interpolate_shape", text="Shape")
        col.prop(brush.curves_sculpt_settings,
                 "interpolate_point_count", text="Point Count")
...
class VIEW3D_PT_curves_sculpt_parameter_falloff(Panel):
    ...
    def draw(self, context):
        ...
        row = layout.row(align=True)
        row.operator("brush.sculpt_curves_falloff_preset",
