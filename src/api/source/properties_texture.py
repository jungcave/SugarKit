...
class TEXTURE_PT_image_mapping(TextureTypePanel, Panel):
    ...
    def draw(self, context):
        ...
        # use layout, to keep the same location in case of button cycling.
        subcol.prop(tex, "extension")
        ...
        if isinstance(idblock, Brush):
            ...
            if tex.texture_coords == 'UV':
                ...
                if ob and ob.type == 'MESH':
                    col.prop_search(tex, "uv_layer", ob.data,
                                    "uv_layers", text="Map")
                else:
...
class TEXTURE_PT_influence(TextureSlotPanel, Panel):
    ...
    def draw(self, context):
        ...
        if isinstance(idblock, ParticleSettings):
            ...
            col = flow.column()
            factor_but(col, "use_map_velocity",
                       "velocity_factor", "Physics Velocity")
            ...
            col = flow.column()
            factor_but(col, "use_map_kink_amp",
                       "kink_amp_factor", "Kink Amplitude")
            factor_but(col, "use_map_kink_freq",
                       "kink_freq_factor", "Kink Frequency")
            factor_but(col, "use_map_rough", "rough_factor", "Rough")

        elif isinstance(idblock, FreestyleLineStyle):
            col = flow.column()
            factor_but(col, "use_map_color_diffuse",
                       "diffuse_color_factor", "Color")
